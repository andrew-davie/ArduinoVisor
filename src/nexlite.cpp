...

{
    ...
    if (NL_SERIAL.available()) {
        // call it byte at a time and time slice very fast

        ns.buff[bufPos] = NL_SERIAL.read();

        // Serial.print(bufPos);
        // Serial.print("   ");
        // Serial.println(ns.buff[bufPos]);

        if (ns.buff[bufPos] == 0xFF) {
            // could be a footer
            fCnt++;                  // add one to the footer count

            if (fCnt == 3) {
                // 3 footer 0xFF so we have a complete packet now
        ...
        if (bufPos > BUF_SIZE)
            bufPos = 0;                    // this should never happen but just to be safe
...
struct nl_struct *nl_SendCommandWait(void)
{
    ...
    while (fCnt--) {
        // borrrow variable before main serial loop
        delay(1);
    ...
    while (NL_SERIAL.available() > 0) {
        // this is optional, can call it byte as time and work just the same
