...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void nl_Callback(struct nl_struct *ns)
{
    // event from nextion - nl means nextion lite - ns means nextion struct
...
{
    // 0x71 0x80 0x00 0x00 0x00 0xFF 0xFF 0xFF
...
void stuffer()
{
    ...
    if (bytesToStream > 63) {
        // might be more efficient reading larger blocks!
...
void setupIRComparator()
{
    ...
    ACSR |=
        bit(ACIS1) | bit(ACIE) // re-enable interrupts on AC
    ;
...
void play(char *filename, unsigned long seekPoint)
{
    ...
    if (wavInfo(filename)) {
        ...
        TIMSK3 |= (        // Timer3Interrupt Mask Register
            0 | _BV(TOIE3) // ENABLE timer overflow interrupt enable
            );
...
void handleEndOfVideo(int nextMode)
{
    ...
    if (playbackAbsolute >= videoLength) {
        ...
        if (!nextion || customRepeat) {
            // BUT we might have the repeat button ON
            if (debug)
...
void loop()
{
    ...
    if (playing) {
        ...
        if (nextion) {
            if (forceTimePreventSeeker || playbackAbsolute - lastPlaybackAbsolute >= oneSecond) {
                // only update if > 1 second difference
